...
#define CIRCLE_BOUND 0.98 // Added a multiplier for the outter region of a circle
...
typedef std::vector<Vector2> NonConvexPoints;
...
//struct much like the clip rectangle but for only the Y axis
typedef struct _ScanClip
{
	int lowest;			//Y value of smallest polygon vertex
	int highest;		//Y value of largest polygon vertex
} ScanClip;
...
private:
	...
	Colour4 AlphaBlend(const Colour4 colour, const int x, const int y); // Created a method to handle the alpha blend at a certain X and Y position with the colour wanted

	Colour4 LerpColour(const Colour4 start, const Colour4 end, const int step, const int lastStep, const bool alpha); // Created a method for Lerping the colour with a potential alpha lerp
	
	inline int DifferentialIncrement(const int d) { return (d < 0) ? -1 : (d > 0) ? 1 : 0; } // Created an inline method to gather increments from the differentials for my drawline method
